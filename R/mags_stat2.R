...
pathMG= "C:/Users/ioannis.kampouris/Desktop/Botswana_MG/MGs/antismash_predictions"
# list1= data.frame(x= list.files(path = paste0(pathMG), recursive=T, pattern = ".fa"))%>%filter(!str_detect(x,"tmp"))%>%
  # mutate(x=str_replace(x,"faa","tsv"))
# prokkalist=list()
# for(i in 1:length(list1$x)){
#   print(paste0(pathMG,i))
#   file1=read_delim(file =  paste0(pathMG,"/", list1$x[i]), delim =  "\t")%>%mutate(MAG=paste0(list1$x[i]))
#   prokkalist=rbind(file1,prokkalist)
# }
...
stats_classifications=as.data.frame( read_excel(paste0("C:/Users/ioannis.kampouris/Desktop/Botswana_MG/mags_stat.xlsx"), sheet = "Sheet7"))
...
write.csv(stats_complete_mags2, 
file ="C:/Users/ioannis.kampouris/Desktop/Botswana_MG/mags_stat_total.csv" )
...
MRG1MAG2B=dcast(MRG1MAG2, Completeness+Contamination+ Domain+ Genes_No+MAG+Phylum~., value.var = "MAG", fill=0, length)#%>%
MRG1MAG2B=full_join(MRG1MAG2B, MRG1MAG2)
MRG1MAG2B$.[is.na( MRG1MAG2B$.)]=0

MRG1MAG2B$Sample=as.numeric( gsub("final*.*","", MRG1MAG2B$MAG))
MRG1MAG2B$Genes_No=as.numeric(MRG1MAG2B$Genes_No )

MRG1MAG2C=MRG1MAG2B%>%filter(
   Completeness>80&Contamination<10)
...
MRG1MAG3=filter(MRG1MAG2, MAG%in%c(CARDMAGs3$MAG))
...
g2= full_join(select(CARDMAGs2, DrugClass, Class, Order, Family,Genus,  ARG=`AMR.Gene.Family`, MAG, Phylum ),
...
gg2=ggplot(g2, aes(axis1=MAG2, axis2= DrugClass, axis3=HGTM, axis4=Compound))+
...
gXX= full_join(select(CARDMAGs2, Class, Order, Family,Genus,  ARG=`AMR.Gene.Family`, MAG, Phylum ),
...
ggsave(filename = paste0(pathMG, "NumberofMAGs.png"), width = 15, height = 5)
