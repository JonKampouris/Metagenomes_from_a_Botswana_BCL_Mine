...
ARG1=read.table(paste0(pathMG, "concARGs.txt"))%>%
  mutate(Category="ARG")%>%
  dcast(V1+V2+V7+V8~., value.var = "V11", min)
MGE1=read.table(paste0(pathMG, "concMGEs.txt"))%>%
  mutate(Category="HGT-Markers")
MRG1=read.table(paste0(pathMG, "concBACMetPD.txt"))%>%
  mutate(Category="MRG")
joined1= full_join(MRG1, ARG1, by=c("V1"))%>%
  select(V1, ARG= "V2.x", MRG="V2.y",everything() )%>%na.omit()
